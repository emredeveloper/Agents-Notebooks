...
if api_key:
    # API anahtarı varsa web arama araçlarını kullan
    available_tools = {'web_search': web_search, 'web_fetch': web_fetch}
    tools = [web_search, web_fetch]
    print("Web arama araçları etkinleştirildi (API anahtarı bulundu)")
else:
    # API anahtarı yoksa sadece yerel model kullan
    available_tools = {}
    tools = []
    print("Web arama araçları devre dışı (API anahtarı bulunamadı)")
    print("Sadece yerel model kullanılacak")
...
while True:
  response = chat(model='qwen3:4b', messages=messages, tools=tools, think=True)
  ...
  if response.message.tool_calls and available_tools:
